...
class GBlock(nn.Module):
    """
      ...
      (2) Bilinear Upsample *2 (공간 크기 2배)
      (3) ResidualBlock 1: 채널 (1.5*C_out)
      (4) ResidualBlock 2: 채널 (1.5*C_out)
    """
    def __init__(
        self,
        C_in: int,
        C_out: int,
        Cardinality: int = 2,      #파라미터, 연산량 절감
        ExpansionFactor: int = 2,  #1.5 -> 3
        KernelSize: int = 3   #표준 convolution 크기
    ):
        super().__init__()
        # 블록 입력 채널 = 1.5 * C_in
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # (2) Bilinear 2× 업샘플 (공간 크기 2배) #weight 미리 학습할 필요 없고, 복잡도가 낮아서 interpolate 활용
        x = F.interpolate(
            x,
            scale_factor=2,
            mode="bilinear",
